...
class DataProcessor(object):
    ...
    def execute(self, action, path):
        with transaction.atomic():
            if action == ".merge":
                ...
                raise ValueError(
                    "Invalid action extension {0}. Must be '.merge' or '.split'".format(
                        action
                    )
                )
    ...
    def insert(self, path):
        with open(path, "r") as f:
            ...
            audit_command = models.AuditCommand.objects.create(
                action=models.AuditCommand.Action.MANUAL, creator=self.creator
            )
            for name in names:
    ...
    def delete(self, path):
        with open(path, "r") as f:
            ...
            self.model.objects.filter(name__in=names).log_delete(
    ...
    def split_record(self, name, new_names):
        ...
        with transaction.atomic():
            audit_command = models.AuditCommand.objects.create(
                action=models.AuditCommand.Action.SPLIT, creator=self.creator
            )
            ...
            new_records = [
                self.model.objects.log_get_or_create(
                    audit_command=audit_command, name=new_name
                )[0]
                ...
                for new_record in new_records:
                    through_model.objects.log_get_or_create(
                        ...
                        **{
                            ...
                            self.through_id_name: new_record.id,
                        },
                    )
    ...
    def merge_records(self, names, new_name):
        with transaction.atomic():
            audit_command = models.AuditCommand.objects.create(
                creator=self.creator, action=models.AuditCommand.Action.MERGE
            )
            ...
            canonical_record, created = self.model.objects.log_get_or_create(
                audit_command=audit_command, name=new_name
            )
            payload = {self.through_id_name: canonical_record.id}
            for publication in publications:
                payload["publication_id"] = publication.id
                self.through_model.objects.log_get_or_create(
                    audit_command=audit_command, **payload
                )
            records_to_merge.exclude(name=new_name).log_delete(
                audit_command=audit_command
            )
