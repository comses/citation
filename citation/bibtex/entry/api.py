...
def guess_author_str_split(author_str):
    ...
    author_split_and = [
        models.Author.normalize_author_name(author_str)
        for author_str in author_split_and
    ]
...
def guess_author_email_str_split(author_email_str: str):
    author_emails = [
        author_email.strip()
        for author_email in author_email_str.split("\n")
...
def match_authors_with_emails_by_order(
    authors: List[models.Author], emails: List[str]
) -> List[str]:
...
def match_authors_with_emails_as_linear_assignment_problem(
    authors: List[models.Author], emails: List[str]
):
    ...
    for author_ind in author_inds:
        for email_ind in email_inds:
            ...
            logger.debug(
                "author {} fuzzy matched with email {}".format(author.name, email)
            )
            authors[author_ind].email = emails[email_ind]
    return []


def combine_author_info(
    author_names, author_emails, author_orcids, author_researcherids
) -> Tuple[List[models.Author], List[str]]:
    ...
    if unassigned_emails:
        unassigned_emails = match_authors_with_emails_as_linear_assignment_problem(
            authors, unassigned_emails
        )
...
def create_detached_raw(entry):
    return models.Raw(key=models.Raw.BIBTEX_ENTRY, value=entry)
...
def augment_authors(audit_command, publication, detached_authors):
    if publication.is_primary:
        ...
        for detached_author in detached_authors:
            duplicate = detached_author.duplicates(
                publications__id=publication.id
            ).first()
            if duplicate:
        ...
        logger.debug(
            "augmented %i of %i authors",
            len(augmented_authors),
            publication.creators.count(),
        )
...
def create_authors(
    ...
    for detached_author in detached_authors:
        ...
        if duplicate:
            merger.augment_author(duplicate, detached_author, audit_command)
            models.PublicationAuthors.objects.log_get_or_create(
    ...
    publication_authors = [
        models.PublicationAuthors(publication=publication, author=author)
        for author in unduplicated_authors
    ]
...
def create_container(
    audit_command: models.AuditCommand, detached_container: models.Container
):
...
def get_keywords(entry):
    ...
    keywords += [
        rk.strip().capitalize() for rk in raw_keywords_plus.split(";") if rk.strip()
    ]
    return keywords
...
def attach_keywords(publication, keywords):
    for k in keywords:
        ...
        pts, pts_created = models.PublicationTags.objects.get_or_create(
...
def process(entry: Dict, creator: User, duplicate_pk=None):
    ...
    audit_command = models.AuditCommand(
        creator=creator, action=models.AuditCommand.Action.MERGE
    )
    if publication_already_in_db:
        logger.debug("aleady in db")
        publication = duplicate_publications[0]
        unaugmented_authors = augment_authors(
            audit_command, publication, detached_authors
        )
        ...
        merger.augment_container(
            publication.container, detached_container, audit_command
        )
    ...
    return common.PublicationLoadErrors(
        raw=detached_raw,
        audit_command=audit_command,
...
class MergePublication:
    ...
    def _get_longest(xs, field):
        return getattr(
            max(
                sorted(xs, key=operator.attrgetter(field)),
    ...
    @classmethod
    def final_container_changes(
        cls, final: models.Container, others: Sequence[models.Container]
    ):
    ...
    @classmethod
    def final_raws(
        cls, final: Sequence[models.Raw], others: Sequence[Sequence[models.Raw]]
    ):
    ...
    def merge_publications(cls, publications, audit_command):
        ...
        if other_pubs:
            container_pks, content = cls.final_container_changes(
                kept_pub.container, [p.container for p in other_pubs]
            )
            models.SuggestedMerge.merge_containers(
                pks=container_pks, content=content, audit_command=audit_command
            )
            models.Raw.objects.filter(publication__in=other_pubs).log_update(
                audit_command, publication_id=kept_pub.id
            )
            models.CodeArchiveUrl.objects.filter(publication__in=other_pubs).log_update(
                audit_command, publication_id=kept_pub.id
            )
            discarded_pks = [p.pk for p in other_pubs]
            models.Publication.objects.filter(pk__in=discarded_pks).log_delete(
                audit_command
            )
        return kept_pub
...
def _regen_from_raw(
    raw: models.Raw, creator: User, publication: models.Publication, audit_command
):
    entry = raw.value
    detached_publication = create_detached_publication(entry, creator)
    detached_container = create_detached_container(entry)
...
def regen_from_raws(publications: Sequence[models.Publication], creator: User):
    ...
    load_warnings = []
    for raw in raws:
        warnings = _regen_from_raw(
            raw, creator, publication, audit_command=audit_command
        )
        if warnings:
