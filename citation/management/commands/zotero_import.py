...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument("--test", default=False, help="used for test cases only")
        parser.add_argument(
            "--group",
            dest="group_id",
            default="284000",
    ...
    def get_creators(self, data):
        ...
        for c in data["creators"]:
            creator, created = Author.objects.get_or_create(
                family_name=c["lastName"].strip(), given_name=c["firstName"].strip()
            )
            if not created:
    ...
    def set_tags(self, data, item):
        for t in data["tags"]:
            ...
            if sliced_key == "cod":
                try:
                    item.code_archive_url = re.search(
                        "(?P<url>https?://[^\s]+)", value
                    ).group("url")
                    ...
                    logger.exception(
                        "URL %s could not be parsed for publication_id %s",
                        value,
                        item.pk,
                    )
            # match for email
            ...
            # match for docs
                ...
                model_documentation = ModelDocumentation.objects.get_or_create(
                    name=value
                )[0]
                models.PublicationModelDocumentations.objects.create(
                    publication=item, model_documentation=model_documentation
                )
            # match for platform
            elif sliced_key == "pla":
                platform = Platform.objects.get_or_create(name=value)[0]
                models.PublicationPlatforms.objects.create(
                    publication=item, platform=platform
                )
            # match for sponsor
            elif sliced_key == "spo":
                sponsor = Sponsor.objects.get_or_create(name=value)[0]
                models.PublicationSponsors.objects.create(
                    publication=item, sponsor=sponsor
                )
            elif key:
                logger.debug(
                    "Tag [%s :: %s] was added as is for publication_id %s",
                    key,
                    value,
                    item.pk,
                )
        ...
        try:
            ...
            logger.exception(
                "Exception while saving tags %s %s for publication_id %s",
                item,
                item.title,
                item.pk,
            )
    ...
    def create_container_article(self, data, meta):
        ...
        models.PublicationAuthors.objects.bulk_create(
            [
                models.PublicationAuthors(publication=article, author=author)
                for author in self.get_creators(data)
            ]
        )
        ...
        if not tags:
            ...
            if not article.code_archive_url:
                ...
                try:
                    ...
                    logger.exception(
                        "Error verifying code archive url %s for publication_id %s",
                        article.code_archive_url,
                        article.pk,
                    )
                    article.status = Publication.Status.REVIEWED
        article.save()
    ...
    def process(self, data):
        for item in data:
            ...
            logger.debug(
                "Generating bibliographic entry of type %s for %s",
                publication_type,
                item,
            )
    ...
    def handle(self, *args, **options):
        ...
        if input_data_file:
            ...
            if options["collection_id"]:
                json_data = zot.everything(
                    zot.collection_items(options["collection_id"])
                )
            else:
