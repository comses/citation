...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        publication_mixed_case_dois = Publication.objects.exclude(
            doi__exact=Lower("doi")
        ...
        for publication in publication_mixed_case_dois:
            ...
            if doi:
                doi = doi.lower()
                Publication.objects.filter(id=publication.id).log_update(
                    audit_command=audit_command, doi=doi
                )
