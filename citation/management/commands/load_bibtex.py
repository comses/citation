...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument("-f", "--filename", help="BibTeX file to load into the DB")
        parser.add_argument(
            "-d", "--directory", help="Directory of data files to load into the DB"
        )
        parser.add_argument(
            "-u",
            "--username",
    ...
    def process_bibtex_file(self, path, user):
        ...
        if invalid_output_file.exists():
            logger.debug(
                "deleting old invalid_output_file file %s", invalid_output_file
            )
            invalid_output_file.unlink()
        errors_and_duplicates = bibtex_api.process_entries(
            path.absolute().as_posix(), user
        )
