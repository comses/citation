        ...
        for log in contributor_logs:
            ...
            for count in contributor_count:
                if count["id"] == log["id"]:
                    temp.update(
                        id=log["id"],
                        contribution="{0:.2f}".format(
                            log["contribution"] * 100 / count["count"]
                        ),
...
def initialize_publication_code_platform_cache():
    ...
    distribution_data = generate_aggregated_distribution_data(
        {"is_primary": True, "status": "REVIEWED"},
        RelationClassifier.GENERAL.value,
        "Publications",
    )
    code_archived_data = generate_aggregated_code_archived_platform_data(
        {"is_primary": True, "status": "REVIEWED"}
    )
...
def initialize_network_cache():
    ...
    sponsors = (
        ...
        .values("sponsors__name")
        .order_by("sponsors__name")
        .annotate(count=Count("sponsors__name"))
        .values("count", "sponsors__name")
        .order_by("-count")[:10]
    )
    for sponsor in sponsors:
        sponsors_name.append(sponsor["sponsors__name"])
    sponsors_filter = {
        "sponsors__name__in": sponsors_name,
        "is_primary": True,
        "status": "REVIEWED",
    }
    network_data = generate_network_graph(
        sponsors_filter, NetworkGroupByType.SPONSOR.value
    )
    cache.set(
        CacheNames.NETWORK_GRAPH_GROUP_BY_SPONSORS.value, network_data.graph, 86410
    )
    cache.set(
        CacheNames.NETWORK_GRAPH_SPONSOS_FILTER.value, network_data.filter_value, 86410
    )
    logger.info(
        "Network cache for group_by sponsors completed using static filter: "
        + str(sponsors_name)
    )
    ...
    cache.set(
        CacheNames.NETWORK_GRAPH_TAGS_FILTER.value, network_data.filter_value, 86410
    )
    logger.info(
        "Network cache for group_by tags completed using static filter: "
        + str(tags_name)
    )
