...
class TestPublicationMerge(TestCase):
    pub_merger = SuggestedMerge.MergePublication

    def create_pubs(self, count, counter):
        return [Publication(id=counter()) for _id in range(count)]

    def create_creators(self, count, counter):
        return [Author(id=counter()) for _id in range(count)]

    def create_platforms(self, count, counter):
        return [Platform(id=counter()) for _id in range(count)]

    def create_sponsors(self, count, counter):
        return [Sponsor(id=counter()) for _id in range(count)]

    def create_raws(self, count, counter):
        return [Raw(id=counter()) for _id in range(count)]

    def create_tags(self, count, counter):
        return [Tag(id=counter()) for _id in range(count)]

    def test_container_merge(self):
        pks, content = self.pub_merger.final_container_changes(
            Container(id=5),
            [Container(id=3, name='Foo'), Container(id=6, name='Bar'),
             Container(id=2, issn='aaa')])
        self.assertSetEqual(set(pks), {2, 3, 5, 6})
        self.assertEqual(content, {'name': 'Foo', 'eissn': None, 'type': None, 'issn': {'aaa', }})

    def test_final_citation(self):
        counter = Counter()
        results = self.pub_merger.new_citations(self.create_pubs(10, counter),
                                                [self.create_pubs(5, counter), self.create_pubs(15, counter)])
        self.assertEqual(len(results), 15)

        results = self.pub_merger.new_citations(self.create_pubs(5, counter), [[], [], []])
        self.assertEqual(results, None)

        results = self.pub_merger.new_citations([], [[], [], []])
        self.assertEqual(results, None)

    def test_final_creators(self):
        counter = Counter()
        creators = self.pub_merger.final_creators(
            self.create_creators(10, counter),
            [
                self.create_creators(5, counter),
                self.create_creators(15, counter)
            ]
        )
        self.assertEqual(len(creators), 15)

        creators = self.pub_merger.final_creators(
            self.create_creators(5, counter),
            [[], [], []]
        )
        self.assertEqual(creators, None)

        creators = self.pub_merger.final_creators([], [[], [], []])
        self.assertEqual(creators, None)

    def test_final_platforms(self):
        counter = Counter()
        model = Platform
        constructor = self.create_platforms
        method = self.pub_merger.final_platforms

        # test disjoint
        ps = method(constructor(5, counter),
                    [constructor(5, counter), constructor(10, counter)])
        ...
        # test overlap
        ps = method([model(id=1), model(id=2)],
                    [[model(id=2), model(id=3)], self.create_platforms(5, counter)])
        self.assertEqual(len(ps), 8)

    def test_final_raws(self):
        counter = Counter()
        model = Raw
        constructor = self.create_raws
        method = self.pub_merger.final_raws
    ...
    def test_final_sponsors(self):
        counter = Counter()
        model = Sponsor
        constructor = self.create_sponsors
        method = self.pub_merger.final_sponsors
    ...
    def test_final_tags(self):
        counter = Counter()
        model = Tag
        constructor = self.create_tags
        method = self.pub_merger.final_tags
    ...
    def test_publication_merge(self):
        # ensure one publication remains

        # test that garbage is present because that is part of the spec

        # test garbage collection

        # ensure db is free of garbage after gc
