...
class TestAuthorParsing(TestCase):
   ...
   Gallagher, Daniel/0000-0001-7291-5558"""
        orcid_numbers_split = bibtex_entry_api.guess_orcid_numbers_str_split(
            orcid_numbers_str
        )
        self.assertEqual(
            [
   ...
   """
        researcherid_numbers_split = bibtex_entry_api.guess_researcherid_str_split(
            researcherid_numbers_str
        )
        self.assertEqual(
            [("B-6509-2015", "Chu", "Eric"), ("O-6464-2015", "Chu", "Eric")],
   ...
   lena.sanders@parisgeo.cnrs.fr"""
        author_email_split = bibtex_entry_api.guess_author_email_str_split(
            author_email_str
        )
        self.assertEqual(
            [
...
class TestEntryParsing(TestPublication):
    def test_walderr2013(self):
        user = User.objects.create_user(
            username="bar", email="a@b.com", password="test"
        )
...
class TestNameNormalization(TestCase):
    def test_normalize_name(self):
        ...
        self.assertEqual(
            last_name_and_initials_str,
            util.last_name_and_initials(util.normalize_name(abbas_period)),
        )
        self.assertNotEqual(
            last_name_and_initials_str,
            util.last_name_and_initials(util.normalize_name(abbas_caps_no_middle)),
        )
        self.assertEqual(
            last_name_and_initials_str,
            util.last_name_and_initials(util.normalize_name(abbas_full)),
        )

        last_name_and_initial_str = util.last_name_and_initial(
            " ".join(last_name_and_initials_str)
        )
